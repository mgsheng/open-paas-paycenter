...
//			JOptionPane.showMessageDialog(null, "导出成功!");
	...
	public static void exportBankPayment(HttpServletResponse response,List<MerchantOrderInfo> bankPayments) {
		// 声明一个工作薄
				HSSFWorkbook wb = new HSSFWorkbook();
				// 声明一个单子并命名
				HSSFSheet sheet = wb.createSheet("StoringTalentExcel");  
				...
				cell.setCellStyle(style);
				// 向单元格里填充数据
				for (short i = 0; i < bankPayments.size(); i++) {
					row = sheet.createRow(i + 1);
					row.createCell(0).setCellValue(bankPayments.get(i).getPaymentName());//支付银行名称
					row.createCell(1).setCellValue(bankPayments.get(i).getCountOrderAmount());//交易金额
					row.createCell(2).setCellValue(bankPayments.get(i).getCountPayAmount());//营收金额
					row.createCell(3).setCellValue(bankPayments.get(i).getCountPayCharge());//手续费
					row.createCell(4).setCellValue(bankPayments.get(i).getDimension());//统计维度
					row.createCell(5).setCellValue(bankPayments.get(i).getFoundDate());//日期
				}
				try {
					response.setContentType("application/vnd.ms-excel");
					String filedisplay = "银行缴费报表.xls";
					String iso_filename = parseGBK(filedisplay);
					response.addHeader("Content-Disposition", "attachment;filename="+ iso_filename);
					ServletOutputStream out = response.getOutputStream();
					wb.write(out);                                                      
