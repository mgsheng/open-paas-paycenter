...
//			JOptionPane.showMessageDialog(null, "导出成功!");
	...
	public static void exportOrderOffline(HttpServletResponse response,List<MerchantOrderOffline> offlines) {
		...
		for (short i = 0; i < offlines.size(); i++) {
			...
			row.createCell(4).setCellValue(offlines.get(i).getAppName());//业务来源
			row.createCell(5).setCellValue(offlines.get(i).getSourceUid());//用户ID
			row.createCell(6).setCellValue(offlines.get(i).getSourceUserName());//用户名
			row.createCell(7).setCellValue(offlines.get(i).getRealName());//真实姓名
			row.createCell(8).setCellValue(offlines.get(i).getPhone());//手机号
			row.createCell(9).setCellValue(offlines.get(i).getChannelName());//支付方式
			row.createCell(10).setCellValue(offlines.get(i).getBankName());//发卡行
			row.createCell(11).setCellValue(offlines.get(i).getOperator());//操作人
			row.createCell(12).setCellValue(offlines.get(i).getRemark());//备注
		}
		try {
			response.setContentType("application/vnd.ms-excel");
